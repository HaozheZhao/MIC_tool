...
VIDEO_PATH='./data/ivqa/howto100mqa_nointersec_uniform_sampled/'
class IVQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cur_video = join(VIDEO_PATH,f"{video_path}_{start}_{end}.webm")
        out_dict['image'] =cur_video 
        out_dict['question'] = datum['question']
        out_dict['answer'] = " or ".join([str(datum['answer1']), str(datum['answer2']), str(datum['answer3']), str(datum['answer4']), str(datum['answer5'])])
        return out_dict
...
def get_loader(data_path, args=None, split='train', mode='train',
    ...
    _dset = IVQA2Dataset(data_path,split, verbose)

    dataset = IVQAFineTuneDataset(
        split,
        data_path =data_path,
        raw_dataset=_dset,
        rank=gpu,
...
class IVQA2Dataset:
    ...
    def __init__(self, IVQA_dir,splits: str, verbose=True):
        self.name = splits
        self.splits = splits.split(',')[0]

        # Loading datasets to data
        self.data = pd.read_csv(join(IVQA_dir,f"{self.splits}.csv"))

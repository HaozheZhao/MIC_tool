...
IMG_PATH='./data/ocrvqa/images'
class QCRVQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        re =[]
        key,datum = self.data[idx]

        ###### Image ######
        cur_img = join(IMG_PATH,f"{key}.jpg")
...
def get_loader(data_path, args=None, split='train', mode='train',
    ...
    _dset = QCRVQA2Dataset(data_path,split, verbose)

    dataset = QCRVQAFineTuneDataset(
        split,
        data_path =data_path,
        raw_dataset=_dset,
        rank=gpu,
...
class QCRVQA2Dataset:
    ...
    def __init__(self, QCRVQA_dir,splits: str, verbose=True):
        self.name = splits
        ...
        for split in self.splits:
            self.data.extend(
                self.read_json(join(QCRVQA_dir,'dataset.json')).items())

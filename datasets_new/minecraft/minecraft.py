...
IMG_PATH='./data/minecraft/image/'
class MINECRAFTFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        out_dict['question'] = datum['question_split']
        out_dict['answer'] = datum['caption']
        return out_dict
...
def get_loader(data_path, args=None, split='train', mode='train',
    ...
    _dset = MINECRAFT2Dataset(data_path,split, verbose)

    dataset = MINECRAFTFineTuneDataset(
        split,
        data_path =data_path,
        raw_dataset=_dset,
        rank=gpu,
    ...
    loader.task = 'MINECRAFT'

    return loader


class MINECRAFT2Dataset:
    ...
    def __init__(self, MINECRAFT_dir,splits: str, verbose=True):
        self.name = splits
        ...
        self.data = self.read_json(join(MINECRAFT_dir,'filter_cap.json'))['annotations']

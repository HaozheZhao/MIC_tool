...
class STVQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ###### Image ######
        source = self.split
        img_path = datum['file_path']
        cur_img = join(self.data_path,img_path)

        out_dict['image'] =cur_img 
        out_dict['question'] = datum['question']
        # out_dict['answer'] = ' or '.join(datum['answers'])
        out_dict['answer'] = random.sample(datum['answers'],1)[0]
        return [out_dict]
...
def get_loader(data_path, args=None, split='train', mode='train',
    ...
    _dset = STVQA2Dataset(data_path,split, verbose)

    dataset = STVQAFineTuneDataset(
        split,
        data_path =data_path,
        raw_dataset=_dset,
        rank=gpu,
...
class STVQA2Dataset:
    ...
    def __init__(self, STVQA_dir,splits: str, verbose=True):
        self.name = splits
        ...
        for split in self.splits:
            for i in range(3):
                self.data.extend(
                    self.read_json(join(STVQA_dir,f'{split}_task_{1+i}.json'))['data'])
        if verbose:

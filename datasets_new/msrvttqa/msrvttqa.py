...
class MSRVTTQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cur_video = join(VIDEO_PATH,f"{split_path}/video{video_path}.mp4")
...
def get_loader(data_path, args=None, split='train', mode='train',
    ...
    _dset = MSRVTTQA2Dataset(data_path,split, verbose)

    dataset = MSRVTTQAFineTuneDataset(
        split,
        data_path =data_path,
        raw_dataset=_dset,
        rank=gpu,
    ...
    loader.task = 'MSRVTTQA'

    return loader


class MSRVTTQA2Dataset:
    ...
    def __init__(self, MSRVTTQA_dir,splits: str, verbose=True):
        self.name = splits
        ...
        self.data = []
        self.data.extend(
            self.read_json(join(MSRVTTQA_dir,f"{self.splits}_qa.json")))
